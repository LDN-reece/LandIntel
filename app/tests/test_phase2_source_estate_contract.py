...
class Phase2SourceEstateContractTests(unittest.TestCase):
    ...
    def test_urgent_address_title_pack_links_addresses_and_title_candidates_without_ownership_certainty(self) -> None:
        ...
        self.assertIn("ownership_not_confirmed_until_title_review", MIGRATION)
        self.assertIn("title_number_candidate_not_ownership_confirmation", MIGRATION)
        self.assertIn("candidate_normalized_title_number", MIGRATION)
        self.assertIn("valid_title_number", MIGRATION)
        self.assertIn("!~ '^SCT[0-9]+$'", MIGRATION)
        self.assertIn("case when valid_title_number then coalesce(title_number, candidate_normalized_title_number) else null end as title_number", MIGRATION)
        self.assertIn("validation.validation_status is distinct from 'rejected'", MIGRATION)
        ...
        self.assertIn("api_key", RUNNER)
        self.assertIn("oauth2", RUNNER)
        self.assertIn('key.lower() != "key"', RUNNER)
        self.assertIn("def _is_os_places_auth_failure", RUNNER)
        self.assertIn("os_places_access_unauthorised", RUNNER)
        self.assertIn("attempted_site_count", RUNNER)
